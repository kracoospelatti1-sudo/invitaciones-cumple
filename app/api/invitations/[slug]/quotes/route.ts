...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!items || items.length === 0) {
    return NextResponse.json({ error: "Envia una lista de frases valida." }, { status: 400 });
  }
  ...
  const last = await prisma.invitationQuote.findFirst({
    where: { invitationId: invitation.id },
    orderBy: { sortOrder: "desc" },
  });
  const startOrder = last ? last.sortOrder + 1 : 0;

  await prisma.invitationQuote.createMany({
    data: items.map((phrase, index) => ({
